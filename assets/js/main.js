(function(){"use strict";
 ...
 f&&f.addEventListener("submit",function(e){
   ...
   if(f.email && f.email.hasAttribute("required") && !email){ showError(f.email, "Please enter your email address."); valid=false; }
   else if(email && !emailRe.test(email)){ showError(f.email, "Please enter a valid email address."); valid=false; }
   if(f.message && f.message.hasAttribute("required") && !f.message.value.trim()){ showError(f.message, "Please enter your comments."); valid=false; }
   if(f.company && f.company.hasAttribute("required") && !f.company.value.trim()){ showError(f.company, "Please enter your company name."); valid=false; }
   ...
   var data=new FormData(f); data.append("type", f.getAttribute("data-lead-type")||"enquiry");
   fetch("send-lead.php",{method:"POST",body:data})
     .then(function(r){return r.json();})
